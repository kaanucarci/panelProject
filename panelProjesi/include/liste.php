            ...
            <div class="card">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                    $veriler=$DB->getInfo($kontrol[0]["tablo"], "", "", "ORDER BY ID ASC");
                      if ($veriler!=false) 
                      {
                        $sira=0;
                        for ($i=0; $i < count($veriler) ; $i++) { 
                            $sira++;
                            if($veriler[$i]["durum"]){$aktifpasif='checked = "checked"';}else{$aktifpasif='';}
                            ?>
                            <td><?=$sira?></td>
                            <td><?php 
                            ...
                            <td>
                              <div class="form-group">
                                  <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success">
                                    <input type="checkbox" class="custom-control-input aktifpasif<?=$veriler[$i]["ID"]?>" id="customSwitch3" <?=$aktifpasif?> value="<?=$veriler[0]["ID"]?>" onclick="aktifpasif(<?=$veriler[$i]["ID"]?>, '<?=$kontrol[0]["tablo"]?>'); ">
